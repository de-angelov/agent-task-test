...
describe("ticket details route", () => {
  it("renders the ticket fields, state labels, null epic text, and edit navigation", () => {
    ...
    const html = renderToString(
      <TicketDetailsView
        data={readTicketDetails(database, ticket.id)}
      />,
    );
  ...
  it("renders the state labels for all current ticket states", () => {
    ...
    const backlogTicket = createTicketForTest({
      teamId: team.id,
      state: "backlog",
    });
    const todoTicket = createTicketForTest({
      teamId: team.id,
      state: "todo",
    });
    const inProgressTicket = createTicketForTest({
      teamId: team.id,
      state: "in-progress",
    });
    const doneTicket = createTicketForTest({
      teamId: team.id,
      state: "done",
    });
